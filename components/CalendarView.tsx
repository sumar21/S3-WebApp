...
import { ChevronDown, Search, Eye, Clock, MapPin, User, Stethoscope, Calendar as CalendarIcon, X, Loader2, RotateCw, ChevronRight, ArrowRight, AlertCircle } from 'lucide-react';
...
const REPLACEMENTS_DATA_ENDPOINT = 'https://default20435c5a4f504349a09a856bdf1f70.49.environment.api.powerplatform.com:443/powerautomate/automations/direct/workflows/77a9412ad0304d94a16f72684caadfb8/triggers/manual/paths/invoke?api-version=1&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=hT-hbAbBJPXglNUwME1AXdSBcvzIWEjcHhiT_scqczk';
...
interface AbsenceEvent {
  ...
  Motivo_AS: string;
  CorreoAusencia_AS?: string;
  UserReemplazo_AS?: string; // Field for the doctor who took the replacement
  IDUnicoAusencia_AS?: string; // Unique ID link
}

interface ReplacementData {
  ID: number;
  IDUnicoAusencia_R: string;
  UsuarioReemplazo_R: string;
  FechaReemplazo_R: string;
  Status_R: string;
}
...
export const CalendarView: React.FC = () => {
  ...
  const fetchCalendarData = useCallback(async () => {
    ...
    try {
      // Parallel fetch for events and replacements
      const [eventsRes, replacementsRes] = await Promise.all([
        fetch(CALENDAR_DATA_ENDPOINT, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ Mesinicio: mesInicio }),
        }),
        fetch(REPLACEMENTS_DATA_ENDPOINT, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ Mesinicio: mesInicio }),
        })
      ...
      const replacementsResult = await replacementsRes.json();

      let fetchedEvents: AbsenceEvent[] = [];
      let fetchedReplacements: ReplacementData[] = [];

      // Calendar endpoint often returns data directly or wrapped in body
      if (eventsResult && eventsResult.data && Array.isArray(eventsResult.data)) {
        fetchedEvents = eventsResult.data;
      } else if (eventsResult && eventsResult.body && Array.isArray(eventsResult.body.data)) {
        fetchedEvents = eventsResult.body.data;
      }

      // Replacements endpoint uses body.data based on provided JSON
      if (replacementsResult && replacementsResult.body && Array.isArray(replacementsResult.body.data)) {
        fetchedReplacements = replacementsResult.body.data;
      } else if (replacementsResult && Array.isArray(replacementsResult.data)) {
        fetchedReplacements = replacementsResult.data;
      }
  ...
  const getDayEvents = (day: number): EventDetails[] => {
    ...
    return events
      ...
      .map(e => {
        ...
        // Find the replacement for this event on-the-fly
        const replacement = replacements.find(r => r.IDUnicoAusencia_R === e.IDUnicoAusencia_AS);
        ...
        return {
          ...
          originalData: {
            ...e,
            UserReemplazo_AS: replacement ? replacement.UsuarioReemplazo_R : e.UserReemplazo_AS
          }
        };
      });
  };
  ...
  const hasAbsenceInMonth = (doctorName: string) => {
    return events.some(e => e.User_AS === doctorName && (e.Status_AS === 'Pendiente' || e.Status_AS === 'Tomado'));
  };

  const hasTakenReplacementInMonth = (doctorName: string) => {
    const searchName = doctorName.toLowerCase().trim();
    return replacements.some(r =>
      r.UsuarioReemplazo_R &&
      r.UsuarioReemplazo_R.toLowerCase().trim() === searchName &&
      r.Status_R === 'Tomado'
    );
  };
                ...
                {filteredAvailability.map((doctor) => (
                  <tr key={doctor.ID} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-sm font-medium text-gray-900">
                      <div className="flex items-center gap-2">
                        {hasAbsenceInMonth(doctor.ConcatName_US) && (
                          <div className="p-1.5 bg-orange-100 text-orange-600 rounded-lg border border-orange-200" title="Tiene una ausencia este mes">
                            <AlertCircle size={14} />
                          </div>
                        )}
                        {hasTakenReplacementInMonth(doctor.ConcatName_US) && (
                          <div className="p-1.5 bg-indigo-100 text-indigo-600 rounded-lg border border-indigo-200" title="Tomó un reemplazo este mes">
                            <User size={14} />
                          </div>
                        )}
                        {doctor.ConcatName_US}
                      </div>
                    </td>
